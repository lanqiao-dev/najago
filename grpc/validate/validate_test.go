...
func TestValidator_ValidateGRPCRequest(t *testing.T) {
	...
	translatedValidator, err := NewValidator(
		WithTranslators(en.New(), zh.New()),
		WithRegisterDefaultTranslationFunc(zh.New().Locale(), zh_translations.RegisterDefaultTranslations),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	testCases := []struct {
				...
				st, err := status.New(codes.InvalidArgument, "failed to validate request").
					WithDetails(&errdetails.BadRequest{
						FieldViolations: []*errdetails.BadRequest_FieldViolation{
							{
								Field:       "request.InnerField.Field",
								Description: "Field最小只能为100",
							},
							{
								Field:       "request.RequiredField",
								Description: "RequiredField为必填字段",
							},
						}})
				if err != nil {
					t.Fatal(err)
		...
		},
		{
			...
			request:   "request",
			want:      nil,
			wantErr: &validator.InvalidValidationError{
